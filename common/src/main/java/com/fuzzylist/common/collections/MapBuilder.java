...
public class MapBuilder<K, V> {
     ...
     * Class constructor.
     *
     * @param initialSize Initial size of map, for optimal memory allocation and performance.
     */
     ...
     * @return New builder with empty map.
     */
    ...
    public static <K, V> MapBuilder<K, V> newBuilder(int initialSize, Pair<K, V>... entries) {
        ...
        for (Pair<K, V> entry : entries) {
            builder.map.put(entry.key, entry.value);
        }
    ...
    /**
     * Append additional key/value pair to the map.
     *
     * @param key   Key.
     * @param value Value.
    ...
    public MapBuilder<K, V> with(Pair<K, V> pair) {
        Assert.notNull(pair, "Pair object cannot be null.");
